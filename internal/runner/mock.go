...
// MockRunner implements Runner and NetworkPolicyRunner for tests.
...
// NewMockRunner creates a new MockRunner with a small default ReadyDelay
// to avoid SQLite BUSY errors when the session manager's async goroutine
// updates the DB before the initial HTTP response completes.
...
func (m *MockRunner) Type() Type {
	return TypeMock
}

func (m *MockRunner) CreateWorkload(_ context.Context, config *WorkloadConfig) (*WorkloadResult, error) {
	m.mu.Lock()
	defer m.mu.Unlock()
...
func (m *MockRunner) ListWorkloads(_ context.Context) ([]WorkloadInfo, error) {
	m.mu.Lock()
	defer m.mu.Unlock()

	result := make([]WorkloadInfo, 0, len(m.workloads))
...
var _ NetworkPolicyRunner = (*MockRunner)(nil)
